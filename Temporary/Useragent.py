...
class Useragents:
    def __init__(self) -> None:
        self.DEVICE_LIST=[line for line in list(DEVICE.keys())]
        self.COUNTRY_LIST=[line for line in list(COUNTRY.keys())]
        pass
...
class Useragents1:
    ...
    def device(self):
        return{
            "device":os.popen("getprop ro.product.manufacturer","r").read().strip(),
            "device_brand":os.popen("getprop ro.product.brand","r").read().strip(),
            "device_model":os.popen("getprop ro.product.model","r").read().strip(),
            "device_build":os.popen("getprop ro.product.build.id","r").read().strip(),
            "device_board":os.popen("getprop ro.product.board","r").read().strip(),
            "device_armeabi":os.popen("getprop ro.product.cpu.abi","r").read().strip(),
            "device_country":os.popen("getprop ro.product.locale.region","r").read().strip(),
            "device_version":os.popen("getprop ro.product.build.version.release","r").read().strip(),
            "device_operator":os.popen("getprop gsm.sim.operator.alpha","r").read().strip(),
            "device_sdk_version":os.popen("getprop ro.product.build.version.sdk","r").read().strip()
}
    def threads(self, code=None,version=None,default=False):
        code=code or "489720161"
        version=version or "289.0.0.77.109"
        if default:
            if os.path.isdir("/data/data/com.termux/files/home"):
                device= self.device()
                return f'Barcelona {version} Android ({device["device_sdk_version"]}/{device["device_version"]}; 440dpi; 1080x2254; {device["device"]}/{device["device_brand"]}; {device["device_model"]}; {device["device_board"]}; qcom; in_ID; {code})'
            else:return Useragents().threads()
        else:return Useragents().threads()
            
    def instagram(self,code=None,version=None,default=False):
        code=code or "642371207"
        version=version or "350.0.0.0.35"
        if default:
            if os.path.isdir("/data/data/com.termux/files/home"):
                device= self.device()
                return f'Instagram {version} Android ({device["device_sdk_version"]}/{device["device_version"]}; 440dpi; 1080x2254; {device["device"]}/{device["device_brand"]}; {device["device_model"]}; {device["device_board"]}; qcom; in_ID; {code})'
            else:return Useragents().instagram()
        else:return Useragents().instagram()
